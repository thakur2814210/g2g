		      ...
		      $result = array();
				...
				/*	$delivery = DB::table('zones')->where('zone_id', '=', session('shipping_address')->zone_id)->get();
					...
					*/
					...
					$billing_email            			=   session('billing_address')->billing_email;
  ...
  public function orders($request){
					...
					foreach($orders as $orders_data){
                        ...
                        if(count($orders_products) > 0)
						    ...
						    $orders[$index]->total_price = isset($orders_products->total_price) ? $orders_products->total_price : null;
						...
						$orders_status_history = DB::table('orders_status_history')
                            ...
                            //dd($orders_status_history->count());
                            if($orders_status_history->count() > 0){
                                ...
                                $orders[$index]->orders_status_id = isset($orders_status_history->orders_status_id) ? $orders_status_history->orders_status_id: null;
    						    $orders[$index]->orders_status = isset($orders_status_history->orders_status_name ) ? $orders_status_history->orders_status_id: null;
                            }
