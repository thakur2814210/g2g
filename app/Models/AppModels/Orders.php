...
class Orders extends Model
{
  ...
  public static function currencies($currency_code){
   ...
   public static function telrtoken($request)
{
  ...
  if($authenticate==1){
    ...
    $device_type = isset($request->device_type) ? $request->device_type : 'defualt';
    $device_uuid = isset($request->uuid) ? $request->uuid : 'defualt';
    
    $app_name = isset($request->app_name) ? $request->app_name : 'G2G';
    $app_version= isset($request->app_version) ? $request->app_version : '1.0.0';
    $app_user = $request->client_id;
    $app_id = isset($request->app_id) ? $request->app_id : 'com.g2g.app';
    ...
    if($paymentFor == 'Service Request Payment'){
        ...
        $cartid = $serviceRequests->sr_code;
        $billing_address_pobox = $serviceRequests->pobox;
        $billing_address_line1 =   $serviceRequests->address;
        $billing_address_city = isset($cityList[$serviceRequests->city]) ? $cityList[$serviceRequests->city] : null;
        $billing_address_region = isset($cityList[$serviceRequests->city]) ? $cityList[$serviceRequests->city] : null;
        $billing_address_country = isset($countryList[$serviceRequests->country]) ? $countryList[$serviceRequests->country] : null;
        ...
        $section  = DB::table('sections')
                  ...
                  ->where('sections_description.language_id',$languages_id)
                  ->where('sections.id', $serviceRequests->category->id)
                  ->first();
                  
        $tran_description= $section->name;
         
        
    }elseif($paymentFor == 'Package Subscription Payment'){
        
        $cartid = time();
        $billing_address_pobox = $request->pobox;
        $billing_address_line1 =   $request->address;
        $billing_address_city = isset($cityList[$request->city_id]) ? $cityList[$request->city_id] : null;
        $billing_address_region = isset($cityList[$request->city_id]) ? $cityList[$request->city_id] : null;
        $billing_address_country = isset($countryList[$request->country_id]) ? $countryList[$request->country_id] : null;
        ...
        $tran_description= $request->package_name .' - '.$section->name;
        $tran_amount = $request->tran_amount;
        
    }elseif($paymentFor == 'Product Purchase'){
        
        $cartid = time();
        $billing_address_pobox = $request->pobox;
        $billing_address_line1 =   $request->address;
        $billing_address_city = isset($request->city) ? $request->city : null;
        $billing_address_region = isset($request->city) ? $request->city : null;
        $billing_address_country = isset($request->country) ? $request->country : null;
        $tran_description= 'Shopping';
        $tran_amount = $request->tran_amount;
        
    }else{
        $responseData = array('success'=>'2', 'data'=>array(), 'message'=>"Something went wrong! Please try later.");
  ...
  return $orderResponse;
   ...
   public static function telrpaymentstatus($request)
{
  ...
  if($authenticate==1){
    ...
    $data = json_decode($responseData);
    $responseData = array('success'=>'1', 'data'=>$data, 'message'=>"Telr data found.");
  ...
  if($authenticate==1){
	...
	//						'updatedAt'=>$result->transaction->updatedAt->date,
        ...
        $payments_setting = Orders::payments_setting_for_stripe($request);
			...
			}else if($payment_method == 'telr'){

                
				$paymentMethodName = 'Telr';
				$payment_method = 'Telr Checkout';
				$payment_status='success';
