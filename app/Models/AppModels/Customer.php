    ...
    public static function processlogin($request)
    {
        ...
        if ($authenticate == 1) {
            ...
            if(is_numeric($email)){
               ...
               $loginInfo = array("phone" => $email, "password" => $password);
               $userData = \DB::table('users')->where('phone', $email)->first();
            } else if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
                $loginInfo = array("email" => $email, "password" => $password);
                $userData = \DB::table('users')->where('email', $email)->first();
            } else {
                $loginInfo = array("user_name" => $email, "password" => $password);
                $userData = \DB::table('users')->where('user_name', $email)->first();
            } 
            ...
            if(Auth::attempt($loginInfo)){
                if(!$userData->email_verified){
                    ...
                    $responseData = array('success' => '2', 'data' => $userData->email, 'message' => "We have sent an email with a confirmation link to your email address. In order to complete the sign-up process, please click the confirmation link. If you do not receive a confirmation email, please check your spam folder");
                }else if($userData->status != 1){
                     $responseData = array('success' => '0', 'data' => array(), 'message' => "Sorry, your account is inactive and may not login. Contact administrator to activate it.");
                }else{
                    
                    $existUser = DB::table('users')->where('email', $userData->email)->where('status', '1')->get();
                   ...
                   $responseData = array('success' => '1', 'data' => $existUser, 'message' => 'Welcome! You are now able to access all of the G2G App features. You can close this window and continue using the G2G App');
                }
            }else{
...
/*
            if(is_numeric($login)){
               $customerInfo = array("phone" => $request->email, "password" => $request->password, 'role_id' => 2);
            } elseif (filter_var($login, FILTER_VALIDATE_EMAIL)) {
                $customerInfo = array("email" => $request->email, "password" => $request->password, 'role_id' => 2);
            } else {
                $customerInfo = array("user_name" => $request->email, "password" => $request->password, 'role_id' => 2);
            }   

            //$customerInfo = array("email" => $email, "password" => $password, 'role_id' => 2);
            if (Auth::attempt($loginInfo)) {

                $existUser = DB::table('users')->where('email', $email)->where('status', '1')->get();

                if(is_numeric($login)){
                  $existUser = DB::table('users')->where('phone', $email)->where('status', '1')->get();
                } elseif (filter_var($login, FILTER_VALIDATE_EMAIL)) {
                    $existUser = DB::table('users')->where('email', $email)->where('status', '1')->get();
                } else {
                   $existUser = DB::table('users')->where('user_name', $email)->where('status', '1')->get();
                }   
    ...
    public static function processregistration($request)
    {
        ...
        $firstName = $request->customers_firstname;
        $lastName = $request->customers_lastname;
        $email = $request->email;
        $password = $request->password;
        ...
        if ($authenticate == 1) {
            ...
            if(!empty($userData)){
                ...
                if($userData->email_verified ==  1) {
                    $message ="We have sent an email with a confirmation link to your email address. In order to complete the sign-up process, please click the confirmation link. If you do not receive a confirmation email, please check your spam folder.";
                }
                
                if(strcasecmp($userData->email ,  $email) === 0) {
                     $message ="This email address is already being used by another G2G account.Please use a different email address";   
                }
                
                if($userData->phone ==  $phone) {
                     $message ="This phone is already being used by another G2G account.Please use a different phone number";
                }
                
                if($userData->user_name ==  $userName) {
                     $message ="This username is already being used by another G2G account. Please use a different username.";
                }
                $responseData = array('success' => '0', 'data' => $postData, 'message' =>$message);;
                ...
                $message ="We have sent an email with a confirmation link to your email address. In order to complete the sign-up process, please click the confirmation link. If you do not receive a confirmation email, please check your spam folder.";
                $responseData = array('success' => '1', 'data' => $customers, 'message' =>$message);
            }
            ...
            if (count($existUser) > 0) {
                ...
                $customers_id = DB::table('users')->insertGetId([
                    'user_name' => 'Defualt_'.$customers_telephone,
                    'role_id' => 2,
    ...
    public static function sendVerificationEmail($request)
    {
        ...
        if ($authenticate == 1) {
            ...
            if(!empty($userData) && count($userData) > 0){
                ...
                $message ="We have sent an email with a confirmation link to your email address. In order to complete the sign-up process, please click the confirmation link. If you do not receive a confirmation email, please check your spam folder.";
                $responseData = array('success' => '1', 'data' => array(), 'message' =>$message);
                
            }else{
                 $responseData = array('success' => '0', 'data' => array(), 'message' => "Email does not exist in our records.");
            }
    ...
    public static function updatecustomerinfo($request)
    {
        ...
        if($authenticate==1){
            ...
            if($cehckexist){
                ...
                $userData = DB::table('users')->select('users.*')->where('users.id', '=', $customers_id)->where('status', '1')->get();
                if(!empty($currentPassword) && !empty($password)){
                    if (\Hash::check($currentPassword , $cehckexist->password)) {
                        ...
                        $responseData = array('success'=>'4', 'data'=>$userData, 'message'=>"Password changed successfully. Please login with new password.");
                    }else{
                        $responseData = array('success'=>'2', 'data'=>$userData,  'message'=>"Current Password is wrong. Update discarded.");
                    }
                }else{
                    //update into customer
                    DB::table('users')->where('id', $customers_id)->update($customer_data);
        ...
        if ($authenticate == 1) {
            ...
            if (count($device_id) > 0) {
                ...
                if (count($dataexist) >= 0) {
                    ...
                    // future we, 
                   // $myVar = new AlertController();
                   // $alertSetting = $myVar->createDeviceUserAlert($userData);
                }
            } else {
