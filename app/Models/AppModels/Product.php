...
class Product extends Model
{
  ...
  public static function otherCharges($request){
      ...
      $authenticate = $authController->apiAuthenticate($consumer_data);
        ...
        if ($authenticate == 1) {
            $other_charges = DB::table('other_charges')
                        ...
                        ->select('name as name_en', 'name_ar','is_percentage','changes_on_total','amount','categories_id','categories_id','qty_limit')
                        ->where('status', 1)->get();
            ...
            if(count($other_charges)>0){
                $responseData = array('success'=>'1', 'data'=>$other_charges, 'message'=>"Other charge are returned successfull.");
            }else{
                $other_charges = array();
                $responseData = array('success'=>'0', 'data'=>$other_charges, 'message'=>"Other charge are empty.");
            }
        }else{
  ...
  public static function getCategoryById($request)
  {
       $language_id = $request->language_id;
      $categories_id = $request->categories_id;
      $result = array();
      ...
      if ($authenticate == 1) {
          $categories = DB::table('categories')->where('categories_status', '1')->where('categories.categories_id', $categories_id)->first();
          if ($categories) {
              $responseData = array('success' => '1', 'data' => $categories, 'message' => "Returned all categories.");
          } else {
              $responseData = array('success' => '0', 'data' => array(), 'message' => "No category found.");
          }
      } else {
          $responseData = array('success' => '0', 'data' => array(), 'message' => "Unauthenticated call.");
      }
