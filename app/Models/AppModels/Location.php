...
class Location extends Model
{
    ...
    public static function getClientLocation($request)
    {
        $consumer_data = array();
        $consumer_data['consumer_key'] = request()->header('consumer-key');
        $consumer_data['consumer_secret'] = request()->header('consumer-secret');
        ...
        if ($authenticate == 1) {
            $client_id = self::getClientID($request->customers_id);
            if(!empty($client_id)){
                $client_locations = DB::table('client_locations')
                                ->leftjoin('countries', 'countries.id','client_locations.country_id' )
                                ->leftjoin('cities', 'cities.id','client_locations.city_id' )
                                ->select('client_locations.*', 'countries.name as country','cities.name as city')
                                ->where('client_id', $client_id)->get();
                $responseData = array('success' => '1', 'data' => $client_locations, 'message' => "Returned all locations.");
            }else{
                $responseData = array('success' => '0', 'data' => array(), 'message' => "User does not exist in the records.");
            }
        } else {
            $responseData = array('success' => '0', 'data' => array(), 'message' => "Unauthenticated call.");
        }
    ...
    public static function addClientLocation($request)
    {
        $consumer_data = array();
        $consumer_data['consumer_key'] = request()->header('consumer-key');
        $consumer_data['consumer_secret'] = request()->header('consumer-secret');
        ...
        if ($authenticate == 1) {
            $client_id = self::getClientID($request->customers_id);
            $client_location_address = $request->location_name;
            $client_location_city_id = $request->entry_city_id;
            $client_location_country_id = 1;
            $client_location_latitude = $request->location_latitude;
            $client_location_longitude = $request->location_longitude;
            $client_additional_address = $request->client_additional_address;
            
            $data = [
                'client_id' =>  $client_id ,
                'address' => $client_location_address,
                'city_id' => $client_location_city_id,
                'country_id' => 1,
                'latitude' => $client_location_latitude,
                'longitude' => $client_location_longitude,
                'additional_address' => !empty($client_additional_address) ? $client_additional_address : null,
            ];
            
            $current_client_id = DB::table('client_locations')->insertGetId($data);
            $responseData = array('success' => '1', 'data' => $current_client_id, 'message' => "Saved new location successfully.");
    ...
    public static function updateClientLocation($request)
    {
        $consumer_data = array();
        $consumer_data['consumer_key'] = request()->header('consumer-key');
        $consumer_data['consumer_secret'] = request()->header('consumer-secret');
        ...
        if ($authenticate == 1) {
            ...
            $client_additional_address = $request->client_additional_address;
            $client_address = $request->entry_address;

            if(isset($request->location_name)){
                $client_address = $request->location_name;
            }
            ...
            DB::table('client_locations')
            ->where('client_id', $client_id)
            ->where('id', $client_location_id)
            ->update($data);

            $responseData = array('success' => '1', 'data' => array(), 'message' => "Returned all locations.");
        } else {
            $responseData = array('success' => '0', 'data' => array(), 'message' => "Unauthenticated call.");
        }
        $response = json_encode($responseData);
    ...
    public static function deleteClientLocation($request)
    {
        ...
        if ($authenticate == 1) {
            ...
            $client_location_id = $request->client_location_id;
            DB::table('client_locations')->where('client_id', $$client_id)->where('client_location_id', $client_location_id)->delete();
            
            $client_locations = DB::table('client_locations')->where('client_id', $client_id)->get();
            $responseData = array('success' => '1', 'data' => $client_locations, 'message' => "Returned all locations.");
        } else {
            $responseData = array('success' => '0', 'data' => array(), 'message' => "Unauthenticated call.");
        }
