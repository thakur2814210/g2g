...
class AdminController extends Controller
{
	...
	public function dashboard(Request $request){
		...
		if(!is_null($vendors_id )){
			$vendor = DB::table('vendor_details')->where('vendor_details.user_id', $vendors_id)->first();
		...
		$recentProducts = DB::table('products')
      	...
      	$users = User::where('status',1)->where('role_id',2)->orderBy('id', 'DESC');	
      	//dd($users->count());

		 
  		$result['customers'] = $users->take(5)->get();
  		$result['totalCustomers'] = $users->count();
       ...
       // $c_garagePackageSubscribe = GaragePackageSubscribe::count();
        ...
        $count = ['garage' => $c_garage,];
        return view("admin.dashboard",['pageTitle' => $pageTitle , 'counts' => $count])->with('result', $result);
  ...
  //admins
	...
	public function addadmins(Request $request){
		...
		$adminTypes = DB::table('user_types')->where('isActive', 1)->where('user_types_id','>','1')->get();
  ...
  //update admin
   ...
   public function profile(Request $request){
	...
	public function manageroles(Request $request){
		...
		$adminTypes = DB::table('user_types')->where('user_types_id','>','1')->paginate(50);
