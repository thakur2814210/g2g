...
class OrdersController extends Controller
{
    ...
    public function display()
    {
        ...
        $orders = DB::table('orders')->orderBy('orders.orders_id', 'DESC')
        ...
        return view("garage.Orders.index", $title)->with('listingOrders', $ordersData)->with('result', $result);
    }
    ...
    public function vieworder(Request $request)
    {
        ...
        $errorMessage = array();
        $vendors_id =  auth()->user()->id;
        $orders_products = DB::table('orders_products')->where('orders_products.vendors_id', $vendors_id)
            ->where('orders_products.orders_id', $orders_id)
            ->first();
        if(empty($orders_products))
            return redirect()->route('partner.orders')->with('message', 'Either order does not exist in our database or you are not authorised to view. For further assistance please conatct admin support.');
        ...
        return view("garage.Orders.vieworder", $title)->with('data', $ordersData)->with('result', $result);
    }
        ...
        if ($old_orders_status == $orders_status) {
            ...
            if ($orders_status == '2') {
                ...
                foreach ($orders_products as $products_data) {
                    ...
                    $vendors_id = $products_data->vendors_id;
                    if(!is_null($vendors_id)){
                          $vendor = VendorDetails::where('user_id' , $vendors_id)->first();
                          $vendor->balance = $vendor->balance + ($products_data->products_price);
                          $vendor->save();
                    }
    ...
    public function invoiceprint(Request $request)
    {
        ...
        return view("garage.Orders.invoiceprint", $title)->with('data', $ordersData)->with('result', $result);
