<div id="filters_col">
	<form id="filterForm" method="get" action="{{ route('listings.search')}}">
		...
		<div class="filter_type">
			<button type="submit" class="btn btn-sm  btn-success"><i class="fa fa-check-circle"></i> {{trans('website.Apply')}}</button>
			<button type="reset" class="btn btn-sm btn-danger float-right"><i class="fa fa-times"></i> {{trans('website.Reset')}}</button>

		</div>
		<br/>

		<a data-toggle="collapse" href="#collapseFilters" aria-expanded="false" aria-controls="collapseFilters" id="filters_col_bt">{{trans('website.Filters')}} </a>
		<div class="collapse show" id="collapseFilters">
			<div class="filter_type">
	            <h6>{{trans('website.Distance')}}</h6>
	          	<div class="slidecontainer">
				  <input type="range" min="all" max="500" value="{{$distance_filters}}" class="slider" name="distance_filter" id="distance_filter_range">
				  <br/>
				  <p>{{trans('website.Value')}}: <span id="distance_filter_range_value"></span> {{trans('website.KM')}}<br/>
					 <small>{{trans('website.Please select atleast one city to enable distance range')}}.</small>
				  </p>
			...
			<div class="filter_type">
				<h6>{{trans('website.City')}}</h6>
				<ul>
					<li>
						<label class="container_check">{{trans('website.All')}}
